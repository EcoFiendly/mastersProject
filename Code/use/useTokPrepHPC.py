...
useCorpus = df.useTrade.values.tolist()
del df
useCorpus = list(filter(None, useCorpus))

# save useCorpus
with open("/rds/general/user/yl4220/home/Data/use/useCorpus.pkl", "wb") as f:
    pickle.dump(useCorpus, f)
    f.close()
...
useCorpusGen = nlp.pipe(useCorpus, n_process = 8, batch_size = 800, disable = ["parser", "ner"])

useTokens = []
for doc in useCorpusGen:
    useTokens.append(' '.join([(tok.lemma_) for tok in doc if not tok.is_stop and not tok.is_punct and tok.tag_ != 'NNP']))

useCorpusGen2 = nlp.pipe(useTokens, n_process = 8, batch_size = 800, disable = ["parser", "ner"])

useTokens2 = []
for doc in useCorpusGen2:
    useTokens2.append([(tok.lemma_) for tok in doc if not tok.is_stop])

# build bigram model
bigram = Phrases(useTokens2, min_count = 9, threshold = 125)
bigramMod = Phraser(bigram)
# print(bigramMod[useTokens2[0]])
# # list of bigrams obtained, with their corresponding scores
# bigramsList = []
# for doc in bigram.export_phrases(useTokens2):
#     bigramsList.append(doc)

# bigramsList[0:100]

useTokens3 = bigramMod[useTokens2]

# pickle trainTokens3
with open("/rds/general/user/yl4220/home/Data/use/useTokens3.pkl", "wb") as f:
    pickle.dump(useTokens3, f)
    f.close()

# create dictionary
useDict = corpora.Dictionary(useTokens3)

useDict.filter_extremes(no_below=100, no_above=0.45)

useDict.compactify()

useDict.save("/rds/general/user/yl4220/home/Data/use/useDict.dict")

useBoWCorpus = [useDict.doc2bow(doc) for doc in useTokens3]

corpora.MmCorpus.serialize("/rds/general/user/yl4220/home/Data/use/useBoWCorpus.mm", useBoWCorpus)